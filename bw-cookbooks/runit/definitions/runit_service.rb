...
define :runit_service, :directory => nil, :cookbook => nil, :only_if => false, :finish_script => false, :control => [], :run_restart => true, :active_directory => nil, :owner => "root", :group => "root", :template_name => nil, :start_command => "start", :stop_command => "stop", :restart_command => "restart", :status_command => "status", :logger_options => {}, :options => Hash.new, :env => Hash.new do
  ...
  template "#{sv_dir_name}/log/run" do
    ...
    cookbook params[:cookbook] if params[:cookbook]
    if params[:options].respond_to?(:has_key?)
      variables :options => params[:options], :service_name => params[:name]
    end
  end

  unless params[:logger_options].empty?
    directory "/var/log/runit/#{params[:name]}" do
      owner params[:owner]
      group params[:group]
      mode 0644
      recursive true
    ...
    template "/var/log/runit/#{params[:name]}/config" do
      owner params[:owner]
      group params[:group]
      mode 0644
      source "sv-#{params[:template_name]}-logger-options.erb"
      cookbook params[:cookbook] if params[:cookbook]
      variables :logger_options => params[:logger_options], :service_name => params[:name]
    end
  end
  ...
  ruby_block "supervise_#{params[:name]}_sleep" do
    block do
      Chef::Log.info("Waiting until named pipe #{sv_dir_name}/supervise/ok exists.")
